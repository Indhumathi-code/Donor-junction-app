  ...
  const loadPosts = async () => {
    ...
    try {
      ...
      if (res.status === 'success' && res.data) {
        ...
        let currentLng = null;
        try {
          const locParams = await Location.getLastKnownPositionAsync();
          if (locParams && locParams.coords) {
            currentLat = locParams.coords.latitude;
            currentLng = locParams.coords.longitude;
          }
        } catch (err) {
        ...
        const livePosts = res.data.filter(p => !deletedIds.includes(Number(p.id))).map(post => {
          let dist = 'Unknown';
          if (currentLat && currentLng && post.latitude && post.longitude) {
            const km = getDistanceInKm(currentLat, currentLng, post.latitude, post.longitude);
            if (km) dist = `${km} km`;
          }
  ...
  return (
    <SafeAreaView style={styles.container} edges={['right', 'bottom', 'left']}>
      ...
      <View style={[styles.topBar, styles.topBarRow]}>
        ...
        <TouchableOpacity onPress={() => navigation.navigate('CreatePost', { fromScreen: 'Posts' })}>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <Card style={{ marginHorizontal: 15, marginTop: 10 }} onPress={() => navigation.navigate('Schedule', { post: item })}>
              ...
              <Text style={styles.cardSub}>{item.location} • {item.distance || 'Unknown'}</Text>
              ...
              <View style={[styles.cardFooter, { justifyContent: 'space-between', alignItems: 'center' }]}>
                ...
                {item.mobile && (
                  <TouchableOpacity
                    style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: COLORS.PRIMARY, paddingHorizontal: 12, paddingVertical: 6, borderRadius: 14, gap: 4 }}
                    onPress={() => {
                  ...
                  >
                    ...
                    <Text style={{ color: '#fff', fontSize: 11, fontWeight: 'bold' }}>Chat</Text>
                  </TouchableOpacity>
                )}
              </View>
            </Card>
